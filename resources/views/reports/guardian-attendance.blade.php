<x-admin-layout>
    ...
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{ __('Relatório de Frequência de Responsáveis') }}
        </h2>
    </x-slot>

    <div class="px-6 py-4">
        <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
            <div class="grid items-end grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-4">
                ...
                <div>
                    <label for="guardian_id" class="block mb-1 text-sm font-medium text-gray-700">Responsável</label>
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                new TomSelect("#guardian_id", {
                    ...
                    load: function(query, callback) {
                        ...
                        fetch(`/reports/person-search?term=${encodeURIComponent(query)}&type=guardian`)
    ...
    const guardianId = document.getElementById('guardian_id')?.value;
    ...
    if (guardianId) {
        params.append('guardian_id', guardianId);
    }

    fetch(`/reports/guardian-attendance/data?${params.toString()}`, {
        headers: {
    ...
    .then(data => {
        ...
        if (!data.length) {
            tbody.innerHTML =
                `<tr><td colspan="4" class="px-4 py-4 text-center text-gray-500">Nenhum registro encontrado.</td></tr>`;
        } else {
            data.forEach(item => {
