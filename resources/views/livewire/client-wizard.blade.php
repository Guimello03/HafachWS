            ...
            <div class="space-y-6">
                ...
                @php
                    ...
                    $icons = [
                        ...
                        'check' => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m15.75 15.75-2.489-2.489m0 0a3.375 3.375 0 1 0-4.773-4.773 3.375 3.375 0 0 0 4.774 4.774ZM21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>',
                    ];
                @endphp
                ...
                <div class="flex items-center justify-between mb-2">
                    @foreach ($steps as $index => $stepData)
                        <div class="flex-1 flex items-center {{ $index !== 1 ? 'ml-4' : '' }}">
                            ...
                            <div class="ml-3 text-sm font-medium {{ $step >= $index ? 'text-blue-600' : 'text-gray-500' }}">
                                {{ $stepData['label'] }}
                            </div>
                            @if ($index < count($steps))
                                <div class="flex-1 h-0.5 mx-4 {{ $step > $index ? 'bg-blue-600' : 'bg-gray-300' }}"></div>
                            @endif
                        </div>
                    @endforeach
                </div>
                ...
                @if ($step === 1)
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            @error('clientData.name') <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="email" wire:model.defer="clientData.email"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            @error('clientData.email') <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">CNPJ</label>
                            <input type="text" wire:model.defer="clientData.cnpj"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            @error('clientData.cnpj') <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                        </div>

                        <div class="flex items-center justify-center my-6">
                            <div class="border-t border-gray-300 w-full"></div>
                            <span class="px-3 text-gray-500 text-sm whitespace-nowrap">Login do Administrador</span>
                            <div class="border-t border-gray-300 w-full"></div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="email" wire:model.defer="adminUserData.email"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            @error('adminUserData.email') <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="password" wire:model.defer="adminUserData.password"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            @error('adminUserData.password') <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model.defer="schoolData.name"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            @error('schoolData.name') <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">CNPJ da Escola</label>
                            <input type="text" wire:model.defer="schoolData.cnpj"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5">
                            @error('schoolData.cnpj') <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                        </div>
                ...
                @if ($step === 3)
                    <div class="space-y-6">
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg shadow-md p-6 space-y-6">
                            <div>
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-700 mb-2">Administrador</h3>
                                <p><strong>Email:</strong> {{ $adminUserData['email'] }}</p>
                                <p class="text-sm text-gray-400">(A senha não será exibida por segurança)</p>
                            </div>

                            <div>
                                <h3 class="text-lg font-semibold text-gray-700 mb-2">Escola</h3>
